...
// Run `node js/cli.js help` for the commands.
...
import {
  ...
  explainError, explainTies, tokenTable, audit, formatAudit, trace, formatTrace, GencmuError,
} from "./src/node.js";
...
Usage:
  ...
  gencmu audit [options]             report unreachable rules, replaced and extended
                                     rules, and silent items that change nothing
...
Options:
  ...
  --feature NAME         enable a feature; may be repeated
...
A parse prints the result on standard output and any tie or error, explained,
on standard error. The exit status is 0 for an accepted text, 1 for a text the
dialect does not accept, and 2 for a mistake in the command or a grammar.`;

/** @typedef {{command: string, words: string[], dialect: string, pipeline?: string, features: string[], until?: string, format: string, showElided: boolean, elisionOnly?: boolean, autoFeatures: boolean, trace?: {stage: string, position: number}}} Command */
...
function parseArguments(argv) {
  /** @type {Command} */
  const command = { command: argv[0] || "help", words: [], dialect: "cll", features: [], format: "brackets", showElided: false, autoFeatures: true };
...
function testCommand(command) {
  ...
  for (const c of cases) {
    ...
    const result = dialects.get(name).parse(c.text, { features: c.features || command.features });
