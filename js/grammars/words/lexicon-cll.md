...
The cmavo of *The Complete Lojban Language*, with the selma'o its dictionary gives each of them: 598 words. This document is generated by `scripts/generate-lexicon.py` from `corpus/cll-selmaho.tsv`, which `scripts/extract-cll-selmaho.pl` extracts from the dictionary's `all_words.xml`, with the dictionary's numbered subclasses (`UI3a`, `KOhA7`) collapsed to the selma'o the syntax grammar names. Edit the table, not this file.
