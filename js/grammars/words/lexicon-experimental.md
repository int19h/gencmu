...
The cmavo that jbotci recognises, with jbotci's selma'o memberships for each: 943 words, of which 345 are outside CLL. This document is generated by `scripts/generate-lexicon.py` from `corpus/jbotci-selmaho.tsv`, which `dump-jbotci-lexicon` writes from the pinned jbotci checkout. Edit the table or the script, not this file. It is the lexicon of the experimental and Zantufa pipelines, whose syntax grammars name selma'o that CLL does not have.

Each alternative spells one word in the phonemes of the word grammar, as `lexicon-cll.md` explains, and carries the word's classes. A class that CLL's dictionary also gives the word is a strong tag; a class that only jbotci gives a CLL word, `mo'o` as KOhA beside its CLL MAI, is weak, `?"KOhA"`: the syntax stage may read the word that way, but a parse that reads it under its CLL class wins at the first difference, which is the counterpart of jbotci's warning-only memberships. A word CLL does not have carries all its jbotci classes as strong tags. A word is tagged `indicator` when jbotci's primary class for it is an attitudinal class, UI, CAI, Y, DAhO, FUhO or FUhE; `cu'ei`, whose primary class is BAI, is not, and stays a word of the syntax.

Five words are corrected here, because jbotci's table lists classes that neither the dictionary entry, camxes-exp, Guskant's Zantufa PEG nor jbotci v0 assign:

- `mu'ei` is ROI: the dictionary has ROI (3 votes), as do camxes-exp, Zantufa and jbotci v0; jbotci adds BAI and UI.
- `zu'ai` is KOhA: the dictionary has KOhA7, as do camxes-exp, Zantufa and jbotci v0; jbotci adds BAI.
- `no'oi` is NOhOI: the dictionary has NOhOI, the selbri relative clause of camxes-exp and jbotci v0; pinned jbotci has NOI and UI.
- `ku'oi` is KUhOI: the terminator of the NOhOI clause; jbotci lists the word with no selma'o.
- `su'ai` is PA: the dictionary has PA4, which is jbotci's own choice; camxes-exp follows the competing SE definition; jbotci adds NU.

jbotci's table lists no selma'o for the members of the single-word selma'o, since its enumeration of classes stops short of them. Those that CLL has take their CLL class here (`bi'e`, `bo`, `boi`, `cei`, `do'u`, `doi`, `fe'e`, `fe'u`, `fi'o`, `foi`, `ge'u`, `jai`, `ke`, `ke'e`, `kei`, `ki`, `ku'e`, `ku'o`, `lu'u`, `ma'o`, `me`, `me'u`, `mo'e`, `na'u`, `ni'e`, `nu'a`, `nu'i`, `nu'u`, `pe'o`, `ra'o`, `se'u`, `te'u`, `tei`, `tu'u`, `vu'o`, `zi'e`); the rest are the single-word terminals of the experimental syntax grammar, named after the word, which each carries first, before whatever broader class jbotci's table gives it: `bo'ei` BOhEI, `fi'au` FIhAU, `fi'oi` FIhOI, `xo'i` XOhI, `ma'oi` MAhOI, `zo'oi` ZOhOI, `la'oi` LAhOI, `go'oi` GOhOI, `ia'u` IhAU, `ku'au` KUhAU, `le'ai` LEhAI, `lo'ai` LOhAI, `me'oi` MEhOI, `ra'oi` RAhOI, `sa'ai` SAhAI, `ta'ai` TAhAI, `ze'oi` ZEhOI.

The notation is explained in `notation.md`.
