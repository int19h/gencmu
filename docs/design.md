...
- **Audit**: undefined and unreachable rules, every rule a later document
  replaced or extended, stages without an `%ambiguity-resolution`, and silent items that change nothing, of parts that could never emit a token or sound inside one; a condition that applies to no alternative is not an audit finding but an error of the grammar.
