...
This is what the dialects need. A script document adds its letters to the phoneme grammar's rules with `%extend-rule`; a word family adds the syllables its morphology allows. The experimental syntax is a layer over the CLL syntax: it restates 45 CLL rules, extends 1 and adds 30. The Zantufa syntax is a layer over both, with 13 new rules, 9 extended and 17 redefined, where a Zantufa form generalizes an older one over the same text and the two must not both be live; the redefinition restates the rule with both forms under complementary feature guards. So the Zantufa dialect is the CLL and experimental documents plus one Zantufa document of those changes, instead of a generated copy.
