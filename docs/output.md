...
`op` being `define`, `redefine` or `extend`, and `tags`, `emit` optional. A rule's name is a name, or `#`.
...
`{"union":[TERM...]}`, `{"intersection":[TERM...]}`, `{"if":COND,"then":TERM}`,
...
`{"any":[COND...]}`; `{"all":[COND...]}`; `{"captured":"x"}`, `""` for `$`; `{"if":COND,"then":COND}`.

**Emission**: `{"items":[ITEM...]}`, an item being
`{"capture":"x","tags":TERM}`, `tags` optional, `{"capture":"x","silent":true}`, or
`{"insert":"h"}`; `"capture":""` is `$`.
...
package: `{"format":3,"bootstrap":HASH,"documents":{PATH:{"hash":HASH,"dom":DOM}}}`,
