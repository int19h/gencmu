...
(function (root) {
  ...
  function workerSource() {
    return [
      ...
      "var loader = null;",
      "var dialects = new Map();",
      "function handle(request) {",
      "  if (request.kind === 'load') {",
      "    loader = gencmu.loaderFromSources(request.sources);",
      "    dialects.clear();",
      "    return { version: gencmu.version, documents: Object.keys(request.sources).length };",
      "  }",
      "  var options = request.options || {};",
      "  var path = options.dialect || 'dialects/notation.md';",
      "  if (!dialects.has(path)) dialects.set(path, loader.dialect(path));",
      "  var result = dialects.get(path).parse(request.text, { features: new Set(options.features || []), autoFeatures: true });",
      "  return { version: gencmu.version, result: gencmu.resultJson(result), brackets: gencmu.toBrackets(result),",
      "           tree: gencmu.toTree(result), display: gencmu.prettyJson(gencmu.displayValue(result)) };",
      "}",
      "self.onmessage = function (event) {",
      "  var request = event.data;",
      "  try {",
      "    self.postMessage({ id: request.id, result: handle(request) });",
      "  } catch (error) {",
