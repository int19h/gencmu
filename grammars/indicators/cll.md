...
A `ba'e` before an indicator marks the indicator and goes with it. An indicator run is read as far as it goes: `broda ui nai` attaches both words to `broda`, and the stage's preference among parses, `read-on`, is what makes `nai` part of the run rather than the next word of the syntax. A `ba'e` is a word of BAhE, which the CLL lexicon gives only to `ba'e` and `za'e`.
