## Lojban phonemes from Latin text

This grammar is the first stage of every pipeline: it reads the characters of a text and hands the next stage the phonemes they stand for. Its terminals are characters, each written in straight quotes, and the character classes `alpha`, `digit`, `space`, `mark` and `other` that every character token also carries; what it emits is one token per phoneme, tagged with the phoneme, so that the word grammar above it never sees a character and reads every script alike. This document covers the standard Latin orthography of CLL chapter 3 and the frame of the stage, runs of letters between pauses; `cyrillic.md` and `zbalermorna.md` add their letters to the same rules. The notation is explained in `notation.md`.

The phonemes are the letters of CLL chapter 3: the consonants `b c d f g j k l m n p r s t v x z`, the vowels `a e i o u y`, the apostrophe as `h`, as CLL's own EBNF spells it, and the stressed vowels `A E I O U Y`, a stressed vowel being a separate phoneme so that stress is a position in the word grammar rather than a mark it tests. Two tags stand for what is not a letter: `PAUSE` for a pause of any length, and `FOREIGN` for a character that is not Lojban at all, which the word grammar admits only inside a foreign quote.

### The text and its runs

The text is pauses and runs. A pause is any run of pause characters: the period, question mark, exclamation mark and whitespace that CLL 3.1 names, and any other character that is neither a letter of some script nor a digit, since such a character is not Lojban and separates words as firmly. A run is what stands between two pauses, and is read as one of three things: a run in which every vowel is a capital, which CLL 3.9 says carries no stress mark at all and is folded to lower case; an ordinary run of letters, in which a capital vowel marks stress; or a run containing a letter or digit of no script here, every character of which is foreign. The stage prefers the parse that reads on, so a run is never cut short where a rule would let it continue; in particular a period between two digits is read as the decimal point of one number rather than as a pause.
