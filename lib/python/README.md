...
- `features`: feature names to enable in every stage, besides those the pipeline enables with `<?features?>`.
- `auto_features`: add `sa-su` only where the text needs it, by parsing up to the stage named `words` without it first.
...
`ParseResult`, `Stage`, `Node`, `Token`, `ParseError`, `Action` and `Expected` are dataclasses. Tags are `dict[str, bool]`, `True` for a strong tag; ranges are `(start, end)` tuples, source ranges in code points.
