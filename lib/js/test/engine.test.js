...
for (const file of fs.readdirSync(directory).filter((name) => name.endsWith(".json")).sort()) {
  ...
  test(`engine: ${file}`, () => {
    ...
    if (expect.features !== undefined) assert.deepEqual(outcome.features, expect.features);
    if (outcome.usageError) {
      assert.equal(expect.error, "usage", `unexpected usage error: ${outcome.usageError.message}`);
      return;
    }
    if (expect.warnings !== undefined) assert.deepEqual(outcome.json.warnings || [], expect.warnings);
    if (expect.result) {
