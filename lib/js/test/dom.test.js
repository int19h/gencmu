...
test("a malformed precompiled DOM is a miss, and the document is read instead", () => {
  ...
  for (const dom of [{ format: 4, rules: [{}], directives: [] },
    { format: 4, rules: [{ name: "text", op: "define", alternatives: [{ guards: [], expr: { seq: [] } }], conditions: [], at: [1, 1] }], directives: [] }]) {
    const compiled = JSON.stringify({ format: 4, bootstrap: bootstrapHash, documents: { "g.md": { hash, dom } } });
...
test("nesting deeper than any grammar is refused", () => {
  ...
  const dom = { format: 4, rules: [{ name: "text", op: "define", alternatives: [{ guards: [], expr }], conditions: [], at: [1, 1] }], directives: [] };
  ...
  const emitted = { format: 4, rules: [{ name: "text", op: "define", alternatives: [{ guards: [], expr: { ref: "A" } }], emit: { items: [{ capture: "", tags: term }] }, conditions: [], at: [1, 1] }], directives: [] };
...
test("the check holds a precompiled emission and format to the reader's rules", () => {
  const rule = (emit) => ({ format: 4, rules: [{ name: "text", op: "define", alternatives: [{ guards: [], expr: { capture: "x", expr: { ref: "A" } } }], emit, conditions: [], at: [1, 1] }], directives: [] });
  assert.equal(domProblem(rule({ items: [{ capture: "" }, { capture: "x" }] })), "a malformed emission");
  assert.equal(domProblem(rule({ items: [{ capture: "x", silent: true }] })), "a malformed emission");
  ...
  assert.equal(domProblem({ format: 3, rules: [], directives: [] }), "not a DOM of format 4");
  const tagged = (tags) => ({ format: 4, rules: [{ name: "text", op: "define", tags, alternatives: [{ guards: [], expr: { capture: "x", expr: { ref: "A" } } }], conditions: [], at: [1, 1] }], directives: [] });
  ...
  const alternative = (expr) => ({ format: 4, rules: [{ name: "text", op: "define", alternatives: [{ guards: [], expr }], conditions: [], at: [1, 1] }], directives: [] });
